...
void Parsing::invite(std::string line, Client& client)
{
    ...
    if (holder.size() < 3)
    {
        ...
        std::string msg = client.getName() + " INVITE :Not enough parameters\r\n";
        client.sendMsg(msg);
        return;
    }
    ...
    if (!channel->hasClient(&client))
    {
        ...
        std::string msg = client.getName() + " " + channelName + " :You're not on that channel\r\n";
        client.sendMsg(msg);
        return;
    }
    ...
    if (!searchForClient(target))
    {
        ...
        std::string msg = client.getName() + " " + target + " :No such nick\r\n";
        client.sendMsg(msg);
        return;
    }
    ...
    if (channel->hasClient(targetClient))
    {
        ...
        std::string msg = client.getName() + " " + target + " " + channelName + " :User is already in the channel\r\n";
        client.sendMsg(msg);
        return;
    }
    ...
    std::string reply = ":ircserv 341 " + client.getName() + " " + targetClient->getName() + " " + channel->getName() + "\r\n";
